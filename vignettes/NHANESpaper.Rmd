...
The three tables are:
  ...
  * `QuestionnaireVariables` - one row per variable, across all tables, provides access to the text descriptions in the HTML documentation, e.g.,  Figure \@ref(fig:dmdeduc3)
  * `VariableCodebook` - has one row for each possible value for each variable and describes the coded values for all categorical variables. In Figure \@ref(fig:dmdeduc3) it will have the values in the table headed _Code or Value_.
...
FIXME: RG commented the stuff below out as it seems redundant to Table 2.
...
<!-- 
FIXME: RG would like to include this - it seems relevant...
