...
Database URL: https://www.epiconnector.org
...
Detailed examples illustrating the benefits of accessing NHANES data through the Docker container are available at <https://https://www.epiconnector.org/nhanes-docs/>. Here, we give a few brief examples.
...
**In this example, which is discussed in more detail in the Epiconnector website, we see an increasing trend in the average BMI across cycles in all subgroups. This analysis was done separately for each cycle, but one may also consider performing a combined analysis after pooling participants from multiple cycles. However, substantial thought needs to be put into such analyses. Firstly, one needs to be cautious about this kind of secular trend when performing analyses on pooled data; for example, it would perhaps not make sense to estimate a common "average BMI" using pooled data if indeed the estimand has been changing systematically over cycles. Secondly, when combining sampling weights, one needs to be careful about the reference population used to derive the weights, as these may have changed across cycles. The CDC provides substantial guidance on these issues, which should be consulted before such analyses.**

When combining data across cycles, care is also needed to detect and account for coding inconsistencies. Returning to the `DMDEDUC3` example from Figure \@ref(fig:dmdeduc3), we saw previously that the SAS Label had changed across cycles. Deeper inspection reveals that other typographic changes were also made, and at least one of those (see Table \@ref(tab:dmdeduc)) would affect any analysis. Since the variable is categorical, it is common to create a `factor` to represent it. In such a factor, the levels '10th Grade' and '10th grade' would be considered different, and they would be treated as representing different quantities. This is easily remedied, but it must be detected first.

By systematically going through all variables in NHANES we have identified several serious issues. In one case a variable has changed units of measurement across cycles (`LBCBHC`) while in another the same variable name has been used to represent two entirely different measurements in some cycles (`LBXHCT`). Thankfully, such examples are rare, but we have certainly not found them all.

**Yet other examples are not so easy to find using automated search. For example, urine glyphosate data were published in 2022 and 2023 using samples from three earlier cycles (2013-14, 2015-16, 2017-18). Not all participants had their urine sample analyzed, and to account for the sample selection, the relevant data files included a suitably modified sampling weight variable, as is common in many data files. Unfortunately, this variable has a different name in each data file: `WTSSCH2Y` in `SSGLYP_H`, `WTSSGL2Y` in `SSGLYP_I`, and `WTSSBJ2Y` in `SSGLYP_J`. One may be able to determine automatically that each of these variables is missing from the other two files, but manual checking is needed to decide whether they represent the same quantity and whether these datasets can be combined.** 

A benefit of creating a platform like Epiconnector is to allow users to share these important details systematically, and thereby lessen the chance that a _finding_ is erroneous.
...
As a natural follow-up to our current work, we hope to be able to develop a community resource supporting analysis documents, ideally in the form of notebooks, which could be coded in different languages. These would be dynamic in the sense described in [@RepRes]. Having high-quality tutorials that are based on the tasks that users need to perform, such as the need to account for NHANES survey weights without which most inferences are invalid, **guidance on which weight variables are suitable for which analyses, etc.,** will likely increase the awareness of the need for these methods as well as their adoption.
...
We thank the anonymous reviewers for their valuable feedback, and particularly for pointing us to the glyphosate example. The database design is based on work done at Harvard Medical school with the Center (now Core) for Computational Biomedicine (https://github.com/ccb-hms/nhanes-database) and we have benefited greatly from our interactions with them.  In particular we thank Jason Payne for his work.
